...
# Asyncio
CMD ["sh", "-c", "echo '🧹 Purging queues on startup...' && celery -A tasks.celery_app purge -f && echo '✅ Queues purged, 🔀 starting worker [ASYNCIO]...' && celery -A tasks.celery_app worker --loglevel=info --concurrency=100 -Q celery,ingest,parsing,embedding,finalize,notes -P threads --shutdown-timeout=270"]
