# agents/cold_call/worker_config.py
"""
Worker-class abstraction for the cold-call agent nodes.

Mirrors attack_outline/case_brief worker_config; controlled by a separate env var
so cold-call model tiers can be tuned independently.

WorkerClass tiers:
  "tool_only"   — no LLM; deterministic retrieval, DB writes, JSON parsing
  "worker_low"  — cheapest model; formatting, export, assembly
  "worker_mid"  — default mid-range; extraction, grounding, diversity checks
  "orchestrator"— flagship; orchestration, seed generation, thread building, synthesis
"""
...
DEFAULT_PROVIDER: str = os.getenv("COLD_CALL_AGENT_PROVIDER", "anthropic")
