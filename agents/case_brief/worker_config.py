# agents/case_brief/worker_config.py
"""
Worker-class abstraction for the case-brief agent nodes.

Mirrors the attack_outline worker_config; controlled by a separate env var
so case-brief model tiers can be tuned independently.

WorkerClass tiers:
  "tool_only"   — no LLM; deterministic retrieval, DB writes, JSON parsing
  "worker_low"  — cheapest model; formatting, evidence card labelling, final assembly
  "worker_mid"  — default mid-range; extraction, section writing, grounding, revision
  "orchestrator"— flagship; planning, doctrinal synthesis, drafting, critique
"""
...
DEFAULT_PROVIDER: str = os.getenv("BRIEF_AGENT_PROVIDER", "anthropic")
